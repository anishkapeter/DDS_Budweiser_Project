...
We plotted the number of Missing ABV vs number of non-missing abv's and then looked to see if there is any pattern in the way ABV is missing, by state, by styles, or by breweries.  
```{r}
...
We are investigating to see if the missing IBUs have any pattern by State, Brewery, or Style. It looks like certain Breweries have all their beers missing IBUs, where other breweries have none of their beers missing IBUs.
```{r}
...
We checked both ABV and IBU by Style to see if it was appropriate to impute the missing values, and if Style could be used in that calculation.
```{r}
...
IBUs are  MAR when we checked by state, brewery and style but we know that each style has a small range of IBU so we imputed the missing IBU's with the median IBU value. There were 5 styles missing so we looked them up and imputed the values. ABVs are MCAR and we decided to impute them with the median ABV of each style because we know that ABV is associated with style. 
...
all_styles <- beers_and_breweries %>%
  ...
  summarize(Count=n())
all_styles
# Conversation is saved as a .txt file
...
# ABV by Classification Boxplot
IPAs_and_Ales %>% # Looks like IPAs have higher ABV as well
  ggplot(aes(x=ABV)) +
  geom_boxplot() +
  facet_wrap(~IPA_or_Ale,ncol=1)

# IBU by Classification Boxplot
IPAs_and_Ales %>% 
  ggplot(aes(x=IBU)) +
  geom_boxplot() +
  facet_wrap(~IPA_or_Ale,ncol=1)
...
### Popular styles in Colardo

We created a bar plot of the most popular beers in Colorado.  Since this was the state with the most beers, we thought it would be useful for Budweiser to know the style breakdown there.  It turned out to be similar to the country as a whole.
```{r}
